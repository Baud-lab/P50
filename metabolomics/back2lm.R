...
for (study in c('all','MI','NY')) {
	...
	#changed here: although it says qned it's not anymore
	resids_qned_counts = t(apply(this_filtered_clr_counts, FUN = my_regress, MAR = 1))
...
save(resids_qned_counts_all, resids_qned_counts_MI, resids_qned_counts_NY, file = paste(prefix, '_resids_counts.RData',sep=''))
