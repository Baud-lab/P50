...
h5createGroup(fid,paste("metabo_counts_",code, sep=''))
h5createGroup(fid,paste("metabo_counts_",code,"/row_header", sep=''))

max(nchar(rownames(count_data)))
#10
h5createDataset(file=fid,dataset=paste("metabo_counts_",code,"/row_header/sample_ID", sep=''),dims=dim(count_data)[1],storage.mode='character',size=15)
h5write(obj=rownames(count_data),file=fid,name=paste("metabo_counts_",code,"/row_header/sample_ID", sep=''))

h5write(obj=count_data,file=fid,name=paste("metabo_counts_",code,"/matrix",sep=''))

h5createGroup(fid,paste("metabo_counts_",code,"/col_header",sep=''))
max(nchar(colnames(count_data)))
#28
h5createDataset(file=fid,dataset=paste("metabo_counts_",code,"/col_header/phenotype_ID",sep=''),dims=dim(count_data)[2],storage.mode='character',size=30)
h5write(obj=colnames(count_data),file=fid,name=paste("metabo_counts_",code,"/col_header/phenotype_ID",sep=''))

max(nchar(metadata[,'cage']),na.rm=T)
#6
h5createDataset(file=fid,dataset=paste("metabo_counts_",code,"/row_header/cage",sep=''),dims=dim(metadata)[1],storage.mode='character',size=10)
h5write(obj=metadata[,'cage'],file=fid,name=paste("metabo_counts_",code,"/row_header/cage",sep=''))
