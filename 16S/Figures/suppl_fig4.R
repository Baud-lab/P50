#code adapted from https://github.com/blekhmanlab/universal_microbiome_h2/blob/v1.0/3_h2_model_outputs.R section for Fig2c
#some comments left here are theirs (ie not updated)
...
pC_2 <- ggplot(subset(plotdat, status != "Non-taxa phenotype"), aes(x = variable_order, y = est, fill=component)) +
  ...
  pdf(paste('baboon_fig_',study,'.pdf',sep=''), width = 5, height = 20)
