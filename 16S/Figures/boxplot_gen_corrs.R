...
pdf("/users/abaud/htonnele/PRJs/P50_HSrats/16S/plot/comp_gen_corrs_across_cohorts_bee.pdf", h= 6, w = 10.5)
par(mar=c(5.1,5.1,2.1,2.1))

## Creating fake p-values in while waiting for true ones
#all_VCs$logP = runif(nrow(all_VCs), 0, 5)
#a = 0.05
#sig = which(all_VCs$logP < -log10(a))
#nonsig = which(all_VCs$logP > -log10(a) | all_VCs$logP == -log10(a))
...
beeswarm(all_VCs$corr_Ad1d2 ~ all_VCs$study_pair,
         add=T, 
         #col=bee_col, #bg=alpha(bee_col, 0.5), pch= 21, 
         pch=16,
         cex=1.2,
         method = "swarm", pwcol=cols) # "center", "hex", "square"
dev.off()
q()
...
cols = rep("#396C93", dim(all_VCs)[1])
cols[all_VCs$pv_chi2dof2 < 0.05] = "#E64B35FF"
all_VCs$study_pair_num = as.numeric(all_VCs$study_pair)
...
boxplot(all_VCs$corr_Ad1d2 ~ all_VCs$study_pair, 
        ...
        las = 1,  
        whisklty=3) #,
points(jitter(all_VCs$study_pair_num, factor=1), all_VCs$corr_Ad1d2, ylab='',
       pch=21, cex=1.2, 
       col = cols, bg=alpha(cols, 0.7))
