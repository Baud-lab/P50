...
######### Plot for fig supp 4S - this is not working for now
plot_VDs <- function(h_order, plotdat){
  ...
  pal <- c("#91D1C2FF","#00A087FF", "#3C5488FF")  # cage, maternal, additive
  ...
  #VDplots = list(pC_1 = pC_1, pC_2 = pC_2)
}

# For some reason this is not working...
if("VDplot" %in% type){
  for (study in c("NY","MI","TN1","TN2")){
    #pdf(...)
    #par(...)
    res = data_prep(study)
    VDplots = plot_VDs(h_order =h_order, plotdat = plotdat)
    print(plot_grid(VDplots$pC_1, VDplots$pC_2, align = "hv", ncol = 1, rel_heights = c(1,18)))
    #dev.off()
  }
}
